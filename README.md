...
mission-anomaly-copilot/
├── README.md
├── mission-anomaly-copilot-plan.md     ← Full architecture spec (8 sub-tasks)
│
├── backend/
...
│   ├── .env.example
│   ├── schemas/
│   │   ├── anomaly.py                  ← AnomalyEvent, Severity, TriggerType
│   │   └── orbit.py                    ← SatelliteRecord
│   ├── services/
│   │   ├── telemetry_simulator.py      ← Gaussian noise simulator, anomaly injection
│   │   ├── anomaly_detector.py         ← Isolation Forest + rule-based detection
│   │   ├── anomaly_log.py              ← In-memory event store (last 100 events)
│   │   ├── granite_ai.py               ← WatsonxLLM root cause + mission plan
│   │   ├── orbital_params.py           ← Orbital decay + fuel simulation
│   │   ├── corpus_loader.py            ← LangChain Document loader
│   │   ├── vector_store.py             ← ChromaDB + WatsonxEmbeddings
│   │   ├── rag_copilot.py              ← ConversationalRetrievalChain + sessions
│   │   ├── celestrak_client.py         ← TLE fetch + sgp4 altitude computation
│   │   ├── orbit_intel.py              ← Proximity search, conjunction risk
│   │   └── nasa_client.py              ← DONKI API + fallback seed data
│   ├── routers/
│   │   ├── telemetry.py                ← SSE stream, inject, clear
│   │   ├── anomalies.py                ← CRUD + /explain + /plan
│   │   ├── copilot.py                  ← Streaming RAG chat, sessions
│   │   ├── orbit.py                    ← Nearby satellites, conjunction check
│   │   ├── reports.py                  ← Mission summary + incident reports
│   │   └── nasa.py                     ← Space weather endpoints
│   ├── agents/
│   │   ├── state.py                    ← AgentState TypedDict
│   │   ├── detect_node.py              ← Anomaly validation node
│   │   ├── root_cause_node.py          ← Granite root cause node
│   │   ├── risk_node.py                ← Risk scoring node
│   │   ├── action_node.py              ← Granite mission plan node
│   │   ├── human_review_node.py        ← Human approval gate
│   │   └── graph.py                    ← LangGraph StateGraph pipeline
│   └── data/
│       ├── synthetic_anomaly_cases.json  ← 20 spacecraft anomaly case studies
│       └── nasa_anomaly_summaries.txt    ← 11 real NASA incident summaries
│
└── frontend/
    └── src/
        ├── app/
        ...
        │       ├── page.tsx            ← Mission Control home + 3D globe
        │       ├── alerts/page.tsx     ← Live telemetry charts (6 channels)
        │       ├── copilot/page.tsx    ← Mission Response Planner
        │       ├── incident-workspace/ ← FACT / INFERENCE / RECOMMENDATION
        │       ├── incident-copilot/   ← RAG chat with streaming + citations
        │       ├── orbit/page.tsx      ← Orbit Intelligence + 3D map
        │       ├── nasa/page.tsx       ← NASA space weather
        │       ├── reports/page.tsx    ← Report generation
        │       └── settings/page.tsx   ← Settings management
        ├── components/
        │   ├── 3d/OrbitGlobeCanvas.tsx ← Three.js globe, orbital rings, satellites
        │   ├── enterprise/             ← Card, Badge, Button, KpiCard, etc.
        │   ├── dashboard/              ← KpiMetricsStrip, AiInsightsCard, tables
        │   └── layout/Navbar.tsx       ← Top nav, HUD ticker, command palette
        └── lib/hooks/
            ├── useTelemetryStream.ts   ← SSE hook with fallback simulation
            └── useAnomalies.ts         ← Polling hook with seed data
```
...
|---|---|---|
| GET | `/api/telemetry/stream` | SSE stream — 1 tick/second |
| GET | `/api/telemetry/history` | Last 500 ticks |
| POST | `/api/telemetry/inject` | Inject anomaly (`voltage_drop`, `thermal_spike`, `attitude_drift`, `fuel_leak`, `comm_loss`) |
| POST | `/api/telemetry/clear` | Clear active anomaly |
| GET | `/api/anomalies` | Active anomaly events |
| GET | `/api/anomalies/history` | Full event log |
| GET | `/api/anomalies/{id}` | Single event |
| POST | `/api/anomalies/{id}/explain` | Trigger Granite root cause |
| POST | `/api/anomalies/{id}/plan` | Trigger Granite mission plan |
| POST | `/api/anomalies/{id}/resolve` | Mark resolved |
| POST | `/api/copilot/chat` | Streaming RAG chat (SSE) |
| GET | `/api/copilot/sessions` | List active sessions |
| DELETE | `/api/copilot/sessions/{id}` | Clear session |
| GET | `/api/orbit/status` | Spacecraft orbital parameters |
| GET | `/api/orbit/nearby` | Nearby satellites + conjunction risk |
| GET | `/api/orbit/conjunction-check` | Maneuver window assessment |
| GET | `/api/orbit/refresh` | Force TLE cache refresh |
| GET | `/api/nasa/space-weather` | Consolidated space weather |
| GET | `/api/nasa/solar-flares` | Recent flare events |
| GET | `/api/nasa/cme` | CME events |
| GET | `/api/reports/summary` | Mission summary report |
| GET | `/api/reports/incident/{id}` | Incident detail report |

---

## Judge Criteria Mapping

| Feature | Innovation | Technical Depth | IBM Tool Usage | Real-World Impact |
|---|---|---|---|---|
| Live SSE Telemetry + Isolation Forest | ★★★ | ★★★ | — | ★★★ |
| Granite Root Cause Explainer | ★★★ | ★★★ | ★★★ | ★★★ |
| Granite Mission Response Planner | ★★★ | ★★★ | ★★★ | ★★★ |
| RAG Incident Copilot | ★★★ | ★★★ | ★★★ | ★★★ |
| LangGraph Multi-Agent Pipeline | ★★★ | ★★★ | ★★ | ★★★ |
| CelesTrak Orbit Intelligence | ★★★ | ★★ | — | ★★ |
